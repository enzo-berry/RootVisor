...
InitiateVmx()
{
    ...
    g_GuestState = ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(VIRTUAL_MACHINE_STATE) * g_ProcessorCounts, POOLTAG);
...
LaunchVm(int ProcessorID, PEPTP EPTP)
{
    ...
    KAFFINITY AffinityMask;
    AffinityMask = PowerOfTwo(ProcessorID);
    KeSetSystemAffinityThread(AffinityMask);
    ...
    if ( (VOID*)g_GuestState[ProcessorID].VmmStack == NULL )
    {
    ...
    if ( (PVOID)g_GuestState[ProcessorID].MsrBitmap == NULL )
    {
...
TerminateVmx()
{
    ...
    for ( size_t i = 0; i < g_ProcessorCounts; i++ )
    {
        ...
        __vmx_off();
        MmFreeContiguousMemory((PVOID)PhysicalToVirtualAddress(g_GuestState[i].VmxoRegion));
...
AdjustControls(ULONG Ctl, ULONG Msr)
{
    ...
    Ctl &= MsrValue.Parts.High; /* bit == 0 in high word ==> must be zero */
    Ctl |= MsrValue.Parts.Low;  /* bit == 1 in low word  ==> must be one  */
    return Ctl;
}

VOID
FillGuestSelectorData(PVOID GdtBase, ULONG Segreg, USHORT Selector)
{
    SEGMENT_SELECTOR SegmentSelector = {0};
    ULONG AccessRights;
...
SetupVmcs(VIRTUAL_MACHINE_STATE* GuestState, PEPTP EPTP)
{
    ...
    __vmx_vmwrite(
        ...
        AdjustControls(CPU_BASED_HLT_EXITING | CPU_BASED_ACTIVATE_SECONDARY_CONTROLS, MSR_IA32_VMX_PROCBASED_CTLS));
    __vmx_vmwrite(
    ...
    __vmx_vmwrite(
        VM_EXIT_CONTROLS,
        AdjustControls(VM_EXIT_IA32E_MODE | VM_EXIT_ACK_INTR_ON_EXIT, MSR_IA32_VMX_EXIT_CTLS));
...
MainVmexitHandler(PGUEST_REGS GuestRegs)
{
    ...
    ULONG ExitReasonVM      = 0;
    ULONG ExitQualification = 0;

    __vmx_vmread(VM_EXIT_REASON, &ExitReasonVM);
